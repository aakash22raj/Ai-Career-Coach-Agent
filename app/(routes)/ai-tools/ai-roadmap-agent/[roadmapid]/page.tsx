...
function RoadmapGeneratorAgent() {
  ...
  const GetRoadmapDetails = async() => {
    ...
    // ✅ Convert all node types to 'turbo'
    const fixedNodes = content?.initialNodes?.map((node: any) => ({
      ...node,
      type: 'turbo',
    }));
  ...
  return (
    <div className='grid grid-cols-1 md:grid-cols-3 gap-5'>
      <div className='border rounded-xl p-5'>
        ...
        <Button onClick={() => setOpenRoadmapDialog(true)} className='mt-5 w-full'><Plus /> Create Another Roadmap</Button>
      </div>
